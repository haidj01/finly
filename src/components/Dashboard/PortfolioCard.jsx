...
export default function PortfolioCard() {
  const { alpacaAccount, positions, alpacaKey } = useStore()
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm col-span-2">
      ...
      {!alpacaKey ? (
        <div className="text-sm text-gray-400 py-4 text-center">⚙️ Alpaca API를 연결하면 실제 포지션이 표시됩니다.</div>
      ) : (
        <>
          {/* Hero */}
          ...
          {/* Holdings */}
          {positions.length === 0 ? (
            <div className="text-sm text-gray-400 text-center py-4">보유 포지션이 없습니다.</div>
          ) : (
